...
// RankTable6CF 已經讀取完的轉成MAP格式
var RankTable6CF map[string]*Rank6

// RankTable6CNF 已經讀取完的轉成MAP格式
var RankTable6CNF map[string]*Rank6
...
func InitJSONToMap() {
	...
	jsonFileRankTable6CF, err6 := Ioutil.ReadFile("./json/RankTable6CF.json")
	if err6 != nil {
		Fmt.Println(err6)
	}
	jsonFileRankTable6CNF, err7 := Ioutil.ReadFile("./json/RankTable6CNF.json")
	if err7 != nil {
		Fmt.Println(err7)
	}
	...
	error6 := json.Unmarshal(jsonFileRankTable6CF, &RankTable6CF)
	if error6 != nil {
		Fmt.Println("Unmarshal RankTable6CF failed, ", error6)
		return
	}
	error7 := json.Unmarshal(jsonFileRankTable6CNF, &RankTable6CNF)
	if error7 != nil {
		Fmt.Println("Unmarshal RankTable6CNF failed, ", error7)
		return
	}
